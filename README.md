...
| `codecommit-tag-create.sh` | CodeCommit に注釈付きタグを作成し push（断面の固定）。認証/権限チェック（スイッチロール）対応 |
| `codecommit-tag-clone-s3-upload.sh` | 指定タグを clone→切替し、内容を S3 へアップロード（**統合版**）。認証/権限チェック（スイッチロール＋スイッチバック）対応 |
| `codecommit-tag-clone.sh` | 指定タグを clone→切替するだけ（**単体版**・統合版の clone 部分）。認証/権限チェック（スイッチロール）対応 |
| `codecommit-tag-clone2.sh` | `codecommit-tag-clone.sh` に**世代管理**（`--keep` で世代数を保持）を追加した版。認証/権限チェック（スイッチロール）対応 |
| `s3-upload.sh` | ローカルディレクトリを S3 へアップロードするだけ（**単体版**・統合版のアップロード部分）。認証/権限チェック（スイッチバック）対応 |
| `s3-upload2.sh` | `s3-upload.sh` に**複数 `--src` / `--subdir-per-src`**（src ごとの転送先分割）を追加した版。認証/権限チェック（スイッチバック）対応 |

clone とアップロードは、用途に応じて **統合版 1 本** でも、**単体版 2 本の組み合わせ** でも
実行できます（統合版はそのまま残してあります）。いずれも **`--dry-run`** に対応しています
（破壊的・外向き操作を行わず内容のみ表示）。

すべてのスクリプトが、実行開始時の **AWS 認証チェック** と、必要な **操作権限の確認 + 権限スイッチ**
（未認証/権限不足なら警告して終了、オプションで自動切替）に対応します。詳細は
[認証チェックと権限スイッチ](#認証チェックと権限スイッチ全スクリプト共通) を参照してください。
...
CodeCommit 権限が無いときの `--auto-switch-role` / `--switch-role-script` にも対応します
（[認証チェックと権限スイッチ](#認証チェックと権限スイッチ全スクリプト共通) 参照）。
...
権限チェックは **2 段階** です。**clone 前**に CodeCommit 権限を確認（無ければ
`--auto-switch-role` で**スイッチロール**）し、**アップロード直前**に S3 権限を確認
（無ければ `--auto-switch-back` で**スイッチバック**）します。スイッチロール中は S3 権限が
無いことがあるため、両者を別々のタイミングで確認します
（[認証チェックと権限スイッチ](#認証チェックと権限スイッチ全スクリプト共通) 参照）。
...
- `codecommit-tag-clone.sh`: リポジトリ指定は統合版と同じ（`--repo-url` か `--repo-name`+`--region`）。
  ...
  統合版と違い `--dest` は自動削除されず**ローカルに残ります**。CodeCommit 権限が無いときの
  `--auto-switch-role` / `--switch-role-script`（スイッチロール）に対応します。
- `s3-upload.sh`: `--src`（アップロード元）と `--s3-bucket` が必須。`--s3-prefix`, `--exclude-git`
  （`.git/*` 除外）, `--exclude <pat>`（追加除外・複数可）, `--delete`, `--dry-run`, `-y/--yes` 対応。
  git リポジトリを丸ごとアップロードする場合は `--exclude-git` の付与を推奨します。S3 権限が
  無いときの `--auto-switch-back` / `--switch-back-script`（スイッチバック）に対応します。

---

## 認証チェックと権限スイッチ（全スクリプト共通）

**すべてのスクリプト**が、実行開始時に **AWS 認証状態** と **必要な操作権限** を確認します。
...
2. **権限チェック + スイッチ**
   スクリプトが行う操作に応じて、必要な権限を確認します。

   | スクリプト | 確認する権限 | 権限が無いときの切替 |
   | --- | --- | --- |
   | `codecommit-tag-create.sh` | CodeCommit | スイッチロール |
   | `codecommit-tag-clone.sh` / `codecommit-tag-clone2.sh` | CodeCommit | スイッチロール |
   | `s3-upload.sh` / `s3-upload2.sh` | S3 | スイッチバック |
   | `codecommit-tag-clone-s3-upload.sh` | CodeCommit（clone 前）→ S3（upload 直前） | スイッチロール ＋ スイッチバック |
   ...
   # 統合版: clone は CodeCommit ロール、upload は S3 へスイッチバック
   ./codecommit-tag-clone-s3-upload.sh --repo-name my-repo --region ap-northeast-1 \
     --tag release-2026-06-29 --s3-bucket my-artifacts --s3-prefix snapshots \
     --auto-switch-role --switch-role-script /opt/team/switch-role.sh \
     --auto-switch-back --switch-back-script /opt/team/switch-back.sh --yes
   ```
   ...
   （`--auto-switch-role` / `--switch-role-script` は CodeCommit を操作するスクリプト、
   `--auto-switch-back` / `--switch-back-script` は S3 を操作するスクリプトで有効です。
   統合版 `codecommit-tag-clone-s3-upload.sh` は両方に対応します）
